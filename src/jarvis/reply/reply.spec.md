...
6. Agentic Messages Loop with Dynamic Context
   ...
   - **Parallel batch**: before the per-step resolve, when the next several plan steps are independent (concrete, no `<placeholder>` ⇒ no dependence on a prior result) and every tool is `parallel_safe`, the engine dispatches them concurrently in a single turn via `_execute_parallel_plan_batch` (thread pool), appending results in plan order. This overlaps IO-bound tool round-trips (web search, weather, page fetch) without any extra LLM call. Gated by `planner_parallel_enabled` / capped by `planner_parallel_max`; fails open to the sequential single-step path. See `planner.spec.md` → "Parallel batch execution".
