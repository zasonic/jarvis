...
| `src/jarvis/memory/supermemory_backend.spec.md` | Opt-in cloud memory backend (supermemory): mirror scrubbed diary/facts on write, merge recall into enrichment | Off by default; local is the source of truth; disabled ⇒ no import / no network / no behaviour change; fail-open; only scrubbed text leaves the device |
| `src/jarvis/scheduling/scheduling.spec.md` | Scheduled / recurring / background tasks: SQLite store, pure schedule arithmetic, daemon scheduler thread, scheduleTask/listScheduledTasks/cancelScheduledTask tools | 100% local; LLM resolves the *when* into structured numeric fields (no hardcoded date parsing); fail-soft scheduler; dedicated DialogueMemory so scheduled runs don't contaminate the live conversation |
| `src/jarvis/llm.spec.md` | LLM backend abstraction: Ollama native + any local OpenAI-compatible server (vLLM, llama.cpp, LM Studio, Jan); pure request builders, response normalisation to Ollama shape, process-global backend, warmup/health | 100% local (no cloud providers); Ollama default byte-identical; responses normalised so all call sites stay backend-agnostic; fail-soft to Ollama on misconfig |
